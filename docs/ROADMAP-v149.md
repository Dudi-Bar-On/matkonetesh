...
> - 🟡 **Wave 4** — **UX #3 (menu-builder consolidation, the "biggest single UX win") done (v169):** the guided **wizard is now the single builder** — its preset quick-starts (`מנגל מעורב`, fill-from-favorites…) moved into the wizard picker, the confusing in-wizard "open the other builder" jump button removed, and all "build menu" entries (More sheet, helper answer, AI planner) route to the wizard. The legacy `openMenu` panel is retired as an entry point. **UX #3/#6/#7/#10/#14 done** (v169–v171): builder consolidation, single primary review CTA, clickable wizard steps, collapsed timeline shape controls, and the More sheet regrouped into noun sections. Remaining Wave 4: content polish (cheese descriptions, glossary, citations), the rest of the UX/IA track (UX #12 global home search, UX #13 positive wording + shared AI-loading), UI/arch debt, a11y depth. (Seasoning dedup already done in `build.py`.)

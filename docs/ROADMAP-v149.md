...
> - 🟡 **Wave 4** — **UX #3 (menu-builder consolidation, the "biggest single UX win") done (v169):** the guided **wizard is now the single builder** — its preset quick-starts (`מנגל מעורב`, fill-from-favorites…) moved into the wizard picker, the confusing in-wizard "open the other builder" jump button removed, and all "build menu" entries (More sheet, helper answer, AI planner) route to the wizard. The legacy `openMenu` panel is retired as an entry point. **UX #14 (clickable wizard progress steps) + UX #6 (single primary review CTA) done (v170).** Remaining Wave 4: content polish (cheese descriptions, glossary, citations), the rest of the UX/IA track (UX #7 timeline-shape collapse, UX #10 More-sheet regroup, UX #12 global home search, UX #13 positive wording + shared AI-loading), UI/arch debt, a11y depth. (Seasoning dedup already done in `build.py`.)

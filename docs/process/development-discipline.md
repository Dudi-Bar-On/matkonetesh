...
- **Concurrency:** `workers: process.env.CI ? 2 : 10`, pinned in `playwright.config.ts` (**the config's own comment is authoritative — this doc is a summary and drifts; read the config**). Re-measured **2026-07-23 at 419 tests**: 6/8/10 workers all 3/3 clean; **10** chosen locally (fastest, as reliable as 6/8), **2** on CI (the 4-vCPU runner over-subscribes at 10). Still 3/3 clean at 425 tests (verified 2026-07-23). **Re-measure if the suite grows substantially** (full suite ~3× at a candidate count; require all clean). *(History: 8→6 at ~324 tests under contention, superseded by the 2026-07-23 remeasure. A stale copy of this line — "workers:6 @ 324" — misled a migration implementer into a wrong ceiling diagnosis on 2026-07-23; hence the "read the config" note.)*
...
- **Navigation timeout: `navigationTimeout: 60_000`** (raised from Playwright's 30s default, 2026-07-23). The suite has **154 bare `page.goto`** calls, all waiting for the full `'load'` event; under worker contention `'load'` occasionally stretches past 30s even though the page HAS rendered → an intermittent nav-timeout flake. Raising the too-tight ceiling is the fix (the app loads fine; 60s still catches a genuinely hung nav). A fast load is unaffected — it's a ceiling, not a wait.

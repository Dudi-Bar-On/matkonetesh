...
- [ ] **הפקדות לזיכרון הסוכן** (§10.11/§10.16): שער-התועלת הופעל על כל מקור חיצוני שהקשת מצאה;
      המקורות נקלטו (`python scripts/memsync.py --status` מודבק) או "אין מועמדים" נאמר במפורש.
- [ ] **רענון זיכרון הסוכן** (§10.12): `python scripts/memsync.py`; אימות:
      `node scripts/check-memory-fresh.mjs` ירוק.

...
**תבנית קונקרטית:** בייט מטווח הבקרה C0 שאינו tab/LF/CR — כלומר `[\x00-\x08\x0B\x0C\x0E-\x1F]`

⚠️ **הערה שנכתבה מניסיון בן דקות:** בעת כתיבת השורה הזו הכנסתי בטעות את ששת הבייטים עצמם לקובץ,
כי כתבתי את הטווח בתוך מחרוזת Python לא-גולמית ופייתון **פירש** אותו במקום לכתוב אותו. ‏grep דיווח
`Binary file matches` — וזו הייתה הראיה היחידה. **הכלל הזה הוכיח את עצמו על מי שניסח אותו.**

...
**Wave E — make multi-event genuinely professional:** ⏳ **CORE DONE (v165):** ✅ per-event cart scope — `mk-menuqty` + the cart's "bought" ticks are now namespaced per event (`shop:<scope>:…`), fixing the cross-event under-buying leak; ✅ event names in alarms + exact scope-prefix matching (`timerEventName`, and `evRunningCount` no longer substring-matches so `ev-A` ≠ `ev-A2`); ✅ the combined timeline now schedules each item by that event's *actual* chosen method/order/ready (from its scoped `mk-tlstate`), not a default; ✅ single-smoker equipment-contention flags — overlapping smoke windows across events are detected and called out; ✅ **(v166)** a consolidated cross-event shopping list (E5) — one trip across all events, quantities summed with per-event breakdown, unified `xshop:` ticks, reachable from the combined timeline. ✅ **(v167)** E6 data-safety: `evLoad` no longer loses unsaved work when switching events — it persists an active event's edits back to its record before leaving, snapshots an unsaved *draft* with a restore-undo, and every switch is now explicit (a toast naming the event). *Still open (a deliberate future refactor):* full view-parameterization (render another event's cart/print/timeline without changing the active one) + stage-level tasks in the combined view.

**Wave F — workflow legibility:** ⏳ **MOSTLY DONE (v164):** ✅ "reschedule from now" button on the feasibility warning; ✅ Home "cook in progress" banner (shows on a started plan / running / ringing timers, pulses red when a timer expires); ✅ multi-day items now appear as a prep-ahead advisory in the plan instead of being silently dropped; ✅ voice-cook-from-wizard no longer launches empty (work-plan tasks are built even in the items view); ✅ **(v166)** persistent plan checkboxes (keyed by task identity `wpck:<scope>:<label>`, survive rebuilds) + a now/next cue (the first upcoming task is badged "הבא" and highlighted). *Still open:* a fuller "▶ התחל תוכנית" wiring/rename.

...
CUTS = [
 ...
 # safe 63->71 (A.2, 2026-08-04). 63 is the generic beef figure (USDA 145F) and tongue is an ORGAN:
 # ask.fsis.usda.gov names it explicitly — "kidney, liver, stomach, tongue, and tripe, from red meats
 # ... 160 °F" = 71.1C (corpus 04-askusda-variety-meats). cat is בקר, so no organ rule ever saw this row.
 # The SV leg (70C/24-48h) is untouched and genuinely over-pasteurizes; 71 is the INSTANTANEOUS floor.
 dict(n=22,cat="בקר",heb="לשון בקר",eng="Beef Tongue",kg=1.2,svt=70,svh="24-48",smt=105,smh="2",tgt=70,safe=71,sear="לא",mid="קילוף קרום",wrap="לא",rest=20,rub="מלח+פלפל+כמון",wood="היקורי/אלון",coal="Fogo Premium",diff=4,sot=110,soh="6",somid="עטיפה ב-75°C",saved=4.0),
 ...
 dict(n=75,cat="איברים פנימיים",heb="שקדי עגל",eng="Veal Sweetbreads",kg=0.5,svt=65,svh="2",smt=120,smh="0.4",tgt=65,safe=65,sear="כן",mid="חליטה + קרח",wrap="לא",rest=5,rub="מלח+חמאה+לימון",wood="אלון",coal="Weber Lump",diff=4,sot=120,soh="0.5",somid="חליטה ואז צריבה",saved=0.5),
 dict(n=76,cat="איברים פנימיים",heb="שקדי טלה",eng="Lamb Sweetbreads",kg=0.4,svt=65,svh="2",smt=120,smh="0.4",tgt=65,safe=65,sear="כן",mid="חליטה + קרח",wrap="לא",rest=5,rub="מלח+חמאה",wood="דובדבן",coal="Weber Lump",diff=4,sot=120,soh="0.5",somid="חליטה ואז צריבה",saved=0.5),
 ...
 dict(n=80,cat="איברים פנימיים",heb="מוח עגל",eng="Veal Brain",kg=0.4,svt=65,svh="1.5",smt=110,smh="0.3",tgt=65,safe=65,sear="עדין",mid="חליטה",wrap="לא",rest=5,rub="מלח+חמאה חומה+לימון",wood="ללא",coal="Weber Lump",diff=4,sot=110,soh="0.4",somid="חליטה, מרקם קרמי",saved=0.3),
 # ── ירקות (vegetables) ──
 ...
 dict(n=101,cat="ירקות",heb="ארטישוק",eng="Artichoke",kg=0.4,svt=85,svh="2",smt=200,smh="0.15",tgt=85,safe=0,sear="הרתחה/אידוי→גריל",mid="חליטה 25 דק׳",wrap="לא",rest=3,rub="שמן+שום+לימון+פטרוזיליה",wood="אלון/תפוח",coal="Weber Lump",diff=3,sot=100,soh="0.75",somid="חצוי, חתך כלפי מטה",saved=0.6),
 ...
 dict(n=126,cat="דג",heb="דג חרב",eng="Swordfish Steak",kg=0.5,svt=54,svh="0.75",smt=230,smh="0.1",tgt=60,safe=63,sear="גריל ישיר",mid="אין",wrap="לא",rest=2,rub="שמן זית+לימון+אורגנו",wood="תפוח/אלון",coal="Weber Lump",diff=2,sot=230,soh="0.1",somid="סטייק מוצק, חריטה מוצלבת",saved=0.42,doneness={'default': 'med', 'scale': 'fish', 'levels': {'mr': {'c': 52}, 'med': {'c': 58}, 'well': {'c': 64}}}),
...
GLOSSARY = [
 ...
 ("בטיחות","pH בהתססה","Fermentation pH","ירידת ה-pH בהתססה (ל-≤5.3 ליבש, 4.6–5.2 לחצי-יבש) היא מחסום הבטיחות הראשון בנקניקים מיובשים, לצד הניטריט והייבוש."),
 ...
 ("נקניקים","בישול עדין (חליטה עדינה)","Poach","בישול עדין במים ~75-80°C (בועות קטנות, לא רתיחה) עד טמפ' פנים ~71-72°. השיטה המקצועית לנקניקיות עבות — מבשל אחיד בלי לפצח את הקרום, ואז צריבה קצרה לצבע."),
 ...
 ("פרויקטים","שלב הכנה מול הגשה","Prep vs Finish","מלאכות רבות מתחלקות לשני מועדים: 'הכנה מראש' (טחינה→מילוי→יישון/בישול עדין) ו'סיום' (צלייה/צריבה ביום ההגשה). האפליקציה מפצלת אוטומטית בגבול היישון/מנוחה."),]
...
BUILDS["cut-18"] = dict(
 ...
 variants=[
  ("בליל סטייקהאוס","צ׳אק 70% + בריסקט/שפונדרה 20% + אונטרייב 10%. עומק, שומן ולעיסה — קלאסי לקציצה עבה."),
 ...
 phases=[
  ...
  ("4 · עיצוב עדין","עצב קציצה ביד קלה — אל תדחס! בשר דחוס = מרקם נקניק. צור גומה קלה במרכז כדי שלא יתנפח. אל תוסיף מלח לבליל (מלח בפנים מחלץ חלבון והופך למרקם גומי).",0),
  ("5 · מלח ברגע האחרון","מלח רק את החוץ של הקציצה רגע לפני הצלייה, בנדיבות.",0),
...
def sausage_dry(meat, cure, casing, ferment, dry, note, variants):
    return dict(
      ...
      phases=[
        ...
        ("5 · התססה (עד pH)","החזק "+ferment+" ב-~24°C ולחות 80–90% עד **ירידת pH ל-≤5.3** (יבש) / 4.6–5.2 (חצי-יבש). זהו מחסום הבטיחות הראשון.",0),

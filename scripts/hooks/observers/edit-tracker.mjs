...
import { openState, noteEdit, recordEvent } from '../lib/enforcement-state.mjs';
...
export function observe(input) {
  ...
  try {
    noteEdit(db, sessionId, filePathStr);
    if (filePathStr && UI_BASENAMES.has(basename(filePathStr))) {
      recordEvent(db, { sessionId, kind: 'ui_edit', detail: { filePath: filePathStr } });
    }
  } finally {

...
test('FORM manual: category matrix, manual add, custom sub-type, cancel, back (EN)', async ({ page }) => {
  ...
  // (11) walk the 8 device-form categories: cap field present iff capKey or multiCap; fuel row iff smoker/grill/oven; sub-type opts change.
  // 'other' is a constant accessories CHECKLIST (not a device form) — covered by equipment.spec B6/B7.
  // grinder: true — grinder plates reuse the multiCap mechanism (same as sousvide's baths / stuffer's nozzles).
  const CATS = ['smoker', 'grill', 'oven', 'sousvide', 'vacuum', 'probe', 'grinder', 'stuffer'];
  const HAS_CAP: Record<string, boolean> = { smoker: true, grill: true, oven: true, sousvide: true, vacuum: false, probe: true, grinder: true, stuffer: true };
  const HAS_FUEL: Record<string, boolean> = { smoker: true, grill: true, oven: true, sousvide: false, vacuum: false, probe: false, grinder: false, stuffer: false };
  let prevTypeOpts = '';
  for (const c of CATS) {

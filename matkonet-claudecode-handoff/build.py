...
<footer>
  <div class="footnote">מתכונת · מדריך האש — נבנה מהטבלאות של דודי. הנתונים מקומיים, ללא חיבור לרשת. סימוני ה-checklist נשמרים בדפדפן.<br><b class="foot-stamp" style="color:var(--ember2)">מהדורה 146 · 5.7.26</b></div>
</footer>
...
function openMake(id){
  ...
  const html=`
   ...
   <div class="panel-body">${m.desc?`<p class="itemdesc">${m.desc}</p>`:''}<div class="progress"><i id="prog"></i></div><div id="methodArea"></div><div id="extras"></div></div>`;
  showPanel(html);
...
with open("/home/claude/matkonet/index.html", "w", encoding="utf-8") as f:
    f.write(html)
